...
\begin{revresponse}[We appreciate your handling of the review process.]
    We thank you for coordinating this review. Both reviewers raised important concerns regarding the potential impact of missing data and seasonal biases on our conclusions, which we have addressed thoroughly. Our key revisions include:
    \begin{enumerate}
        \item To directly address concerns about seasonal bias, we conducted PERMANOVA analysis comparing the effects of interannual clusters versus season on community composition. This analysis demonstrates that the interannual signal explains approximately four times more variance than seasonal effects ($R^2 = 15.9\%$ vs.\ $3.7\%$), supporting our interpretation that annually aggregated analyses capture the dominant mode of community variability (see Appendix C).
        \item We expanded our analytical framework to treat Early Cluster~1 (1996–2003) and Late Cluster~1 (2014–2016) as separate entities rather than assuming equivalence based on hierarchical clustering alone. We added statistical comparisons (Wilcoxon rank-sum tests) between these periods to explicitly test whether the post-2014 data support the hypothesis of a return to pre-2004 conditions (Table~\ref{tab:ClustCompWilcox}).
        \item We added a dedicated subsection on missing data (Section~2.4.3) and substantially expanded the limitations discussion (Section~4.3) to address data gaps in the final years of the time series.
        \item Following reviewer suggestions, we removed the variable evaporation and added primary productivity to our analysis.
    ...
    \end{enumerate}
    Importantly, none of these modifications substantially altered our main findings or conclusions, which provides additional confidence in the robustness of our results. However, the reviewers' insightful comments regarding data coverage in the final years prompted us to reconsider our framing. We have accordingly tempered our interpretation, now describing the 2014–2016 period as a community in transition toward earlier conditions, rather than a complete return to the pre-2004 state.
\end{revresponse}
...
\begin{revresponse}
    ...
    Key findings:
    \begin{itemize}
        \item AMO ranked as the top predictor across all the secondary upwelling and rainy season with optimized time lags, for the upwelling period it was slightly outranked by the MEI. Notably, AMO emerged as the strongest predictor for secondary upwelling even without any time lag, indicating a strong linkage to community composition during this period.
        \item MEI v.2 ranked first for primary and second for the secondary upwelling with optimized time lags, suggesting ENSO teleconnections influence both upwelling events.
        \item Silicate and precipitation emerged among the top predictors during secondary upwelling. Given that precipitation peaks during June-August, elevated silicate likely reflects enhanced surface runoff during this period rather than upwelling-driven nutrient supply.
        \item The composition of predictable genera varied by season: primary upwelling models predicted predominantly diatoms alongside dinoflagellates, haptophytes and cyanobacteria; secondary upwelling and rainy season models predicted mostly diatoms and dinoflagellates, with dinoflagellates particularly prominent during the rainy season.
    \end{itemize}
    ...
    While seasonality explains statistically significant variance, its effect size is approximately four times smaller than the interannual signal from our identified clusters. The NMDS ordination does not reveal a distinct community state associated with secondary upwelling Figure~\ref{fig:sup:nmds_season}, Table~\ref{tab:sup:permanova_season_cluster}. This suggests that annually aggregated analysis captures the dominant mode of community variability.
...
\end{revcomment}
\begin{revresponse}
	This section of the Methods is only in reference to Figure~\ref{fig:zscore} and the analysis of interannual variance using z-scores. For this specific analysis the years 1995, 2016, 2017 were excluded, for other parts of our analysis (where no annual averaging was performed) the monthly measurements from 2016 were included.
...
\begin{revresponse}
    ...
    For annually aggregated analyses (z-scores in Figure~\ref{fig:zscore}, hierarchical clustering in Figure~\ref{fig:clustering}a, species turnover in Figure~\ref{fig:clustering}b, and annual means in Figure~\ref{fig:divts}), we excluded years with fewer than six monthly observations (1995, 2016, and 2017). The other years also include some missing monthly data (see \autoref{comment:phyto-coverage} for a more detailed discussion of this), yet our hierarchical clustering uses presence-absence data, which is relatively robust to missing months because a genus need only be detected once within a year to be recorded as present, hence 2016 was included here. Other parts of our analysis (NMDS ordination, density distributions, and gradient forest), were performed with the original monthly resolution and include data from 2016.
    
    Regarding the post-2014 period: although 2014 and 2015 have gaps, both years retain observations from both upwelling and rainy seasons, reducing the risk of seasonal bias. To further assess the robustness of our conclusions, we compared monthly observations across three periods (Early Cluster~1: 1996-2003; Cluster~2: 2004-2013; Late Cluster~1: 2014-2016) using Wilcoxon rank-sum tests (see Table~\ref{tab:ClustCompWilcox}). For key variables including SST, salinity, chlorophyll~$a$, and several functional groups, Early~C1 and Cluster~2 differ significantly, and Cluster~2 and Late~C1 differ significantly, while Early~C1 and Late~C1 do not, a pattern consistent with partial recovery to pre-2004 conditions. Additionally, independent sampling near Margarita Island in the Cariaco Basin from 2002-2018 (\cite{gomez_gaspar_variacion_2025}) show a reduction following 2004 and a return to elevated surface chlorophyll during 2014-2017, corroborating a potential recovery.
...
\begin{revresponse}
    ...
    Regarding mixed layer depth and chlorophyll maximum dynamics: Our references to a deeper chlorophyll maximum during stratified periods are based on previous research (\cite{lorenzoni_characterization_2015, pinckney_phytoplankton_2015}). Our depth-resolved data in Figure~\ref{fig:divts} are consistent with these earlier findings. Other parts of our analysis focus on fully depth-integrated properties to capture large-scale community changes, rather than on the vertical structure of the community. Mixed layer depth in the Cariaco Basin is consistently shallow ($<$35~m; \cite{muller-karger_scientific_2019}) and the 21$^{\circ}$C isotherm depth is more commonly used as a proxy for upwelling intensity in this system. Regarding euphotic zone depth data, there are some reported values; e.g. \cite{pinckney_phytoplankton_2015} document a significant deepening of the euphotic zone, with the average depth (defined as 0.1\% PAR) increasing from 61±18 m (1996–2000) to 75±13 m (2006–2010). We have added a sentence in \lineref{R2:C14} to mention this. 
\end{revresponse}
...
\end{revcomment}
\begin{revresponse}
    We agree that the interpretation of cyclicity depends on the representativeness of the post-2014 data. As detailed in our response to the previous comment, our sensitivity analyses and statistical comparisons (Table~\ref{tab:ClustCompWilcox}) provide support to this finding, as does independent evidence from \cite{gomez_gaspar_variacion_2025}. 
...
\begin{revresponse}
	...
	We have already mentioned the discrete depth intervals at which CARIACO data were sampled at in Section 2.2, but now additionally added that information  (1, 7, 15, 25, 35, 55, 75, and 100 m) to Section 2.3.1. Depth integration was achieved by interpolating to a meter-resolution between these standard depths using the ``unesco'' algorithm from the R ``oce'' package, then taking the mean and multiplying by the specified depth intervals. We hope this answers the reviewers questions.
	
	Regarding mixed layer depth: in the tropical coastal ecosystem of the Cariaco Basin, mixed layer depth calculated from density gradients is consistently shallow ($<$\qty{35}{\meter}; \cite{muller-karger_scientific_2019}). For this reason, the 21°C isotherm depth, which we include in our analysis, is more commonly used as a proxy for upwelling intensity in this system (e.g. \cite{astor_seasonal_2003, lorenzoni_bio-optical_2011, pinckney_phytoplankton_2015}).
...
\end{revcomment}
\begin{revresponse}
    Yes, phytoplankton cell count data were also incomplete for these years. As detailed in our response to your \autoref{comment:phyto-coverage}, where we provide a complete list of missing months for phytoplankton data. Years with very few monthly observations, 1995 ($n=2$) and 2017 ($n=1$), were excluded from annually aggregated analyses for this reason (Section~2.4, \lineref{R1:C12:start}[R1:C12:end]). When calculating annual means we additionally excluded 2016 ($n=5$).
    ...
    \begin{changes}
        Although data are sparse after 2013, with 2016 and 2017 excluded from annual mean calculations due to insufficient monthly coverage, the available observations from 2014 and 2015, which include both upwelling and rainy season months, show a strong increase in nutrients and biological variables, coinciding with a return to negative anomalies in the AMO index and to more upwelling-favorable conditions.
    \end{changes}
    
    As previously mentioned this cyclic pattern was proposed by \cite{muller-karger_scientific_2019}. Our hypothesis of the cyclic pattern rests on two lines of evidence detailed in our response to your ~\autoref{comment:first-rev2}: (1) statistical comparisons showing Late Cluster~1 does not differ significantly from Early Cluster~1 while both differ from Cluster~2 (Table~\ref{tab:ClustCompWilcox}); and (2) independent corroboration from \cite{gomez_gaspar_variacion_2025}. 
...
\begin{revresponse}
    ...
    \begin{enumerate}
        \item Depth-discrete cell counts (at 1, 7, 15, 25, 35, 55, 75, and 100~m) were interpolated to 1~m resolution and integrated over the upper 100~m to obtain a single depth-integrated value per taxon per month.
...
\end{revcomment}
\begin{revresponse}
    We thank the reviewer for this question. The choice of depth-integrated cell counts versus depth-averaged (interpolated) nutrient concentrations reflects the different roles these variables play in the gradient forest analysis and the transformations applied to them.
    
    For predictor variables (nutrients), gradient forest is insensitive to the absolute scale of predictors, as the algorithm identifies splits based on relative changes in community composition across the predictor range. Depth-averaged concentrations (in µM) are therefore equivalent to depth-integrated values for this purpose, while being more intuitive to interpret in the context of known physiological thresholds and literature comparisons.
...
\end{revcomment}
\begin{revresponse}
	We thank the reviewer for this suggestion. To provide concrete context, we quantified the reduction in cell counts by comparing the average abundances of major functional groups during the period following the shift (2004–2013) against the preceding baseline period (1996–2003).
...
\end{changes}
    ...
    We have updated the text to explicitly state these quantitative reductions, defining the comparison periods by year to clearly illustrate the magnitude of the shift before introducing the cluster analysis (see \lineref{R2:C26}).
\end{revresponse}
