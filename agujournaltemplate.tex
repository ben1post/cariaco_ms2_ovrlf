...
The period between 2001 and 2006 lacks coverage in HPLC data, so we have to rely on microscopy counts to deduce changes in the phytoplankton community. This limits the size range of the observable phytoplankton and the measurable diversity through the limited sampling volume \cite{cermeno_sampling_2014}. We can see a marked reduction in the diversity of microphytoplankton ahead of the collapse in abundance, and similarly an increase in microphytoplankton diversity a few years ahead of the return in abundance to previous levels. 
...
When comparing the community between the two clusters...
- In microscopy counts we see a stark reduction in cell counts, which corresponds to a reduced concentration of chlorophyll \textit{a} biomass particularly in the top \qty{25}{meters} of the water column. When including the HPLC pigment data, these patterns are less clear, as the total chlorophyll measurement (check Pinckneys discssion on this again) actually shows an increase...
- Independent spectophotometric measurements of chlorophyll \textit{a} concentration at \qty{20}{meters} depth off the coast of Magarita Island follow the temporal trend of the fluorometric chlorophyll reduction between 2004 and 2014 \cite{gomez_gaspar_variacion_2025}. 
- The community shows a stark reduction in diatom cell counts, but also other functional groups --> shift towards smaller cell sizes, not observable in light microscopy

In the Cariaco basin, wind-driven seasonal upwelling dictates nutrient availability in the upper mixed layer, which, in turn, affects phytoplankton dynamics \cite{ptacnik_diversity_2008, bopp_response_2005}. In particular, diatoms respond to changes in the mixing regime \cite{huisman_reduced_2006}.
Alterations in physical mixing affected phytoplankton dynamics,

- Discuss why there is a shift to smaller cell sizes at low upwelling -> increased nutrient efficiency, cite some papers here.
- Discuss the dominant (most observed) genera per Cluster (TODO with DATA Analysis) and how that relates to the conditions
- Discuss how similar Part 1 and 2 of Cluster 1 are.. Why are they clustered together?
- Discuss why we do not observe a clear distinction in Diversity between the two clusters, but mention the species turnover difference and the community evenness increase.
...
When comparing the environmental conditions between the two clusters...
- When looking at distributions of measured environmental variables, there is a clear trend signifying the reduction in upwelling conditions during the period from 2004 to 2014 (cluster 2).
- We have lower wind speed, lower Isotherm, higher Temp, lower Nuts (except Silicate -> river runoff) lower salinity -> less mixing, higher precipitation.
