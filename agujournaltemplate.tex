...
\subsection{Climate data}
    ...
    As the main indicator of diversity, we used genus richness, thereby reducing taxonomic resolution to make the dataset more homogeneous and more robust to discrepancies in sampling effort \cite{ptacnik_diversity_2008}. Additionally, we calculated the Shannon index $H' = -\sum_i p_i \log_{e} p_i$ and Pielou\'s evenness $J' = -\sum p_i ln( p_i )/ln(H')$ based on the genera present for each interpolated and integrated monthly sampling. 

    %RUE/BDEF ANALYSIS
    To investigates the effect of diversity on ecosystem functioning, we compared measurements of chlorophyll a, primary productivity and Resource Use Efficiency for Nitrate ($RUE_N$) against Genus Richness, Shannon Index and Pielou\'s Index, each integrated at the four depth intervals across each monthly sampling. $RUE_N$ is calculated as the ration between the limiting resource ($NO_3$) and phytoplankton biomass (chlorophyll a).
